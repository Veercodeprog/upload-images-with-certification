...
fn certified_http_request(state: &State, request: HttpRequest) -> HttpResponse {
    let certified_tree = state.certified_tree(&[]);
    let certificate = &certified_tree.certificate;
    let response = state.http_request(request.clone(), certificate, unused_callback());
