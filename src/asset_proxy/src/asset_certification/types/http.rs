...
pub fn response_hash(
    ...
    println!(
        "Computing response hash - Status Code: {:?}, Headers: {:?}, Body Hash: {:?}",
        status_code, certified_headers, body_hash
    );
    ...
    println!(
        "Headers after adding synthetic ':ic-cert-status': {:?}",
        headers
    );
    ...
    println!("Final response hash: {:?}", hash);
